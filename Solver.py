...
class Solver:
    ...
    def solve(self):
        ...
        print(f"Applying GLS - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.GLS()

        SolDrawer.draw("1-GLS.png", self.sol, self.allNodes)
        print(f"Applying ClownMove (seed: 5, factor: 1.01) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.ClownMove(5, 1.01)
        SolDrawer.draw("2-ClownMove.png", self.sol, self.allNodes)
        print(f"Applying ClownMove (seed: 4, factor: 1.01) - Cost now is: {[self.clearCost(),self.sol.cost][1]} ")
        self.ClownMove(4, 1.01)
        SolDrawer.draw("3-ClownMove.png", self.sol, self.allNodes)
        print(f"Applying ClownMove (seed: 3, factor: 1) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.ClownMove(3)
        SolDrawer.draw("4-ClownMove.png", self.sol, self.allNodes)
        print(f"Applying ClownMove (seed: 2, factor: 1.01) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.ClownMove(2, 1.01)
        SolDrawer.draw("5-ClownMove.png", self.sol, self.allNodes)
        print(f"Applying ClownMove (seed: 1, factor: 1.01)  - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.ClownMove(1, 1.01)
        SolDrawer.draw("6-ClownMove.png", self.sol, self.allNodes)

        print(f"Applying threeOPT  - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.threeOPT(5)
        SolDrawer.draw("7-threeOPT.png", self.sol, self.allNodes)

        print(f"Applying Route Reversal - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.reverseRoutes()
        SolDrawer.draw("8-reverseRoutes.png", self.sol, self.allNodes)

        print(f"Applying Random Route Part Reversal (seed: 5) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.randomlyPartlyReverseRoutes(5)
        SolDrawer.draw("9-randomlyPartlyReverseRoutes.png", self.sol, self.allNodes)

        self.clearCost()

        print(f"Applying Tabu - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        self.Tabu()
        ...
        print(f"Applying ClownMove (seed: 3, factor: 1.01) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
        ...
        print(f"Applying Random Route Part Reversal (seed: 1) - Cost now is: {[self.clearCost(),self.sol.cost][1]}")
